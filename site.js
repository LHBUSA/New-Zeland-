...
const HOME_TRANSLATIONS={
  ...
  'PARCEL':'PĀNGA WHENUA',
  'TITLES':'TAITARA',
  'Land Information New Zealand':'Land Information New Zealand',
  ...
  'Active national enrichment baseline':'Kei te haere te whakarangatira ā-motu',
  'loaded':'kua utaina',
  'target':'whainga',
  ...
  'LINZ footprints with capture and imagery provenance as coverage loads nationally.':'Ngā tapuwae LINZ me te takenga hopunga/whakaahua i te wā e utaina ana te kapi ā-motu.',
  ...
  'Building layer as covered':'Paparanga whare ina kapi',
  ...
  'Authoritative New Zealand data sourced from Toitū Te Whenua Land Information New Zealand (LINZ) and other official/public sources where applicable, with source-level provenance preserved by PropData. PropData is independent and is not affiliated with or operated by LINZ or the New Zealand government.':'He raraunga whai mana o Aotearoa i ahu mai i Toitū Te Whenua Land Information New Zealand (LINZ) me ētahi atu puna mana/tūmatanui ina hāngai, ā, ka tiakina e PropData te takenga ki te taumata puna. He motuhake a PropData, ā, kāore i whai hononga, kāore hoki i whakahaerehia e LINZ, e te Kāwanatanga o Aotearoa rānei.',
  'Photography used for product presentation via Unsplash. Property records, identifiers, counts and API examples are PropData/LINZ data, not inferred from photography.':'Ka whakamahia ngā whakaahua Unsplash mō te whakaaturanga hua. Ko ngā pūkete rawa, ngā tautohu, ngā tatauranga me ngā tauira API he raraunga PropData/LINZ, kāore i whakapaetia mai i ngā whakaahua.'
};
...
function localizeHomeText(lang){
  ...
  const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode(node){
    ...
    const english=ORIGINAL_TEXT.get(node).trim();
    return HOME_TRANSLATIONS[english]?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT;
  }});
  const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);
  nodes.forEach(node=>{
    const original=ORIGINAL_TEXT.get(node);
    const english=original.trim();
    const replacement=lang==='mi'?(HOME_TRANSLATIONS[english]||english):english;
...
function injectCurrencyStyles(){
  ...
  style.id='fx-style';
  style.textContent=`
    .currency-console{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:10px;margin:20px auto 0;padding:10px 12px;width:fit-content;max-width:100%;border:1px solid rgba(19,126,90,.18);border-radius:14px;background:rgba(255,255,255,.82);box-shadow:0 8px 28px rgba(10,40,54,.05)}
    .currency-console-label{font-size:10px;font-weight:850;color:#5f7381;letter-spacing:.03em}
    .currency-toggle{display:flex;padding:3px;border:1px solid #d9e5e3;border-radius:10px;background:#eef5f3}
    .currency-btn{border:0;background:transparent;color:#657d7b;padding:7px 11px;border-radius:7px;cursor:pointer;font-size:10px;font-weight:900;letter-spacing:.05em}
    .currency-btn.active{background:#0c6f51;color:#fff;box-shadow:0 5px 14px rgba(12,111,81,.18)}
    .currency-billing-badge{font-size:9px;font-weight:850;color:#0d6f52;background:#e7f7f1;border:1px solid #c7e9dc;padding:6px 8px;border-radius:999px}
    .price .fx-sub{display:block;margin-top:3px;font-size:10px;line-height:1.35;font-weight:700;color:#75868c;letter-spacing:0}
    .price .fx-billed{color:#0c7757}
    .fx-disclosure{max-width:880px;margin:18px auto 0;padding:12px 14px;border:1px solid #dce6e3;border-radius:12px;background:#f8fbfa;color:#667b79;text-align:center;font-size:10px;line-height:1.6}
    .fx-disclosure strong{color:#163b34}
    @media(max-width:620px){.currency-console{width:100%;justify-content:space-between}.currency-console-label{width:100%;text-align:center}.currency-billing-badge{width:100%;text-align:center}}
  `;
...
function initCurrencyUI(){
  ...
  const head=pricing.querySelector('.section-head');
  if(head&&!head.querySelector('.currency-console')){
    const consoleEl=document.createElement('div');
    consoleEl.className='currency-console';
    consoleEl.innerHTML=`<span class="currency-console-label"></span><div class="currency-toggle" role="group" aria-label="Currency display"><button class="currency-btn" data-currency="NZD">NZD</button><button class="currency-btn" data-currency="USD">USD</button></div><span class="currency-billing-badge"></span>`;
    head.appendChild(consoleEl);
    consoleEl.querySelectorAll('.currency-btn').forEach(btn=>btn.addEventListener('click',()=>{
      currentCurrency=btn.dataset.currency;
      try{localStorage.setItem('propdata_nz_currency',currentCurrency)}catch{}
      updateCurrencyUI(currentCurrency,document.documentElement.lang.startsWith('mi')?'mi':'en');
    }));
  }
  if(!pricing.querySelector('.fx-disclosure')){
    const note=document.createElement('div');note.className='fx-disclosure';pricing.querySelector('.wrap')?.appendChild(note);
  }
  let stored='';try{stored=localStorage.getItem('propdata_nz_currency')||''}catch{}
  if(stored==='NZD'||stored==='USD')currentCurrency=stored;
  else currentCurrency=(navigator.language||'').toLowerCase().includes('en-us')?'USD':'NZD';
  updateCurrencyUI(currentCurrency,document.documentElement.lang.startsWith('mi')?'mi':'en');
}
...
function updateCurrencyUI(currency,lang){
  ...
  const label=pricing.querySelector('.currency-console-label');
  const badge=pricing.querySelector('.currency-billing-badge');
  ...
  const plans=[...pricing.querySelectorAll('.pricing-grid .plan')].slice(0,3);
  plans.forEach((plan,i)=>{
    const price=plan.querySelector('.price');if(!price)return;
    const p=PRICE_PLANS[i];
    if(currency==='USD'){
      price.innerHTML=`≈US$${p.usd}<small>/mo</small><span class="fx-sub fx-billed">${lang==='mi'?`Ka namahia hei NZ$${p.nzd} / marama`:`Charged as NZ$${p.nzd} / month`}</span>`;
    }else{
      price.innerHTML=`NZ$${p.nzd}<small>/mo</small><span class="fx-sub">${lang==='mi'?`≈US$${p.usd} i te reiti tohutoro o nāianei`:`≈US$${p.usd} at the current reference rate`}</span>`;
    }
  });
  const disclosure=pricing.querySelector('.fx-disclosure');
  if(disclosure){
    disclosure.innerHTML=lang==='mi'
      ? `<strong>Moni nama: NZD.</strong> Reiti tohutoro ${FX.asOf}: NZ$1 ≈ US$${FX.NZD_TO_USD.toFixed(4)} · US$1 ≈ NZ$${FX.USD_TO_NZD.toFixed(4)}. He tatauranga anake ngā utu USD. Ka whakatau tō pēke, tō whatunga kāri rānei i te utu whakawhiti whakamutunga, ā, tērā pea ka tāpiri utu whakawhiti.`
      : `<strong>Billing currency: NZD.</strong> Reference rate ${FX.asOf}: NZ$1 ≈ US$${FX.NZD_TO_USD.toFixed(4)} · US$1 ≈ NZ$${FX.USD_TO_NZD.toFixed(4)}. USD prices are estimates only. Your bank or card network determines the final converted USD amount and may add foreign-exchange fees.`;
  }
  document.querySelectorAll('.site-footer a[href*="buy.stripe.com"]').forEach((link,i)=>{
    const p=PRICE_PLANS[i];if(!p)return;
    const name=link.textContent.split('·')[0].trim();
    link.textContent=currency==='USD'?`${name} · ≈US$${p.usd} (billed NZ$${p.nzd})`:`${name} · NZ$${p.nzd} (≈US$${p.usd})`;
  });
}
